...
function postJournal() {
    ...
    fetch('http://127.0.0.1:3000/journal/create', {
        method: "POST",
        headers: new Headers({
...
function editJournal(postId) {
    ...
    const fetch_url = `http://127.0.0.1:3000/journal/update/${postId}`;
    const accessToken = localStorage.getItem('SessionToken');

    let card = document.getElementById(postId);
    let input = document.createElement('input');

    if (card.childNodes < 2) {
...
function deleteJournal(postId) {
    ...
    const fetch_url = `http://127.0.0.1:3000/journal/delete/${postId}`;
    const accessToken = localStorage.getItem('SessionToken');
